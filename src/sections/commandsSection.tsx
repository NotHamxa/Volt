...
function ShellSelector({ value, onChange }: { value: ShellChoice; onChange: (v: ShellChoice) => void }) {
    ...
    return (
        <div className="inline-flex items-center gap-0.5 p-0.5 rounded-md bg-fill-040 border border-line-080">
            {opts.map(o => (
                <button
                    key={o.id}
                    type="button"
    ...
    return (
        <div className="space-y-1.5">
            {args.map((a, i) => {
                ...
                return (
                    <div key={i} className="p-2 rounded-md bg-fill-025 border border-line-060 space-y-1.5">
    ...
    return (
        ...
        >
            <span className={`pointer-events-none inline-block ${dot} transform rounded-full shadow transition duration-200 ease-in-out mt-[1px] ${
                checked ? `${trX} bg-surface` : "translate-x-0.5 bg-ink/40"
            }`} />
        </button>
    );
}
        ...
        <label className="flex items-center gap-1.5 text-[10.5px] text-ink/45 cursor-pointer select-none shrink-0 group">
            <button
                ...
                className={`flex items-center justify-center w-3.5 h-3.5 rounded-[3px] border transition-colors ${
                    checked
                        ? "bg-ink border-ink"
                        : "bg-fill-040 border-ink/[0.18] group-hover:border-ink/[0.3]"
                }`}
            >
                {checked && (
            ...
            <div className="grid grid-cols-1 gap-1.5 min-w-0">
                {loading ? (
                    ...
                    filtered.map((cmd, index) => (
                        editingCmd === cmd.name ? (
                            <div key={index} className="p-3 rounded-md bg-fill-040 border border-line-120 space-y-2.5">
                                {/* Rewrites the fields below; the edit still has
                                    to be saved by hand. */}
                                <div className="pb-2.5 border-b border-line-060">
                                    <AiCommandComposer
...
function PresetCommandsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--scrim-strong)] backdrop-blur-[6px]" onClick={onClose}>
            <div
                onClick={e => e.stopPropagation()}
                className="w-[640px] h-[540px] flex flex-col rounded-2xl bg-surface-modal/[0.98] border border-line-070 shadow-[0_40px_90px_var(--shadow-3),inset_0_1px_0_var(--edge-hi)] overflow-hidden"
            >
